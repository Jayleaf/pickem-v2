...
pub async fn decide(ctx: Context, channel_id: &ChannelId, content: &str) {
    ...
    for user in losing_team {
        ...
        match user_record {
            ...
            None => {
                connection.execute("INSERT INTO records (id, category, wins, losses, ties) VALUES (?1, ?2, ?3, ?4, ?5)", params![user.id.to_string(), game.category.to_string(), 0, 1, 0]).expect("Failed to insert record");
            }
        }
    }
}